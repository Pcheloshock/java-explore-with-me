...
echo "Checking stats service (port 9090)..."
for i in {1..30}; do
  if curl -s -f http://localhost:9090/actuator/health 2>/dev/null | grep -q "UP"; then
...
echo "Checking main service (port 8080)..."
for i in {1..60}; do
  if curl -s -f http://localhost:8080/actuator/health 2>/dev/null | grep -q "UP"; then
